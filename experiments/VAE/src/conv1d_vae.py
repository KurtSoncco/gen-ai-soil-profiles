...
class Conv1DVAE(nn.Module):
    ...
    def __init__(
        self, input_dim, latent_dim, hidden_dims=[64, 128, 256], kernel_size=5
    ):
        super().__init__()
        ...
        for hidden_dim in hidden_dims:
            encoder_layers.append(
                ResidualBlock1D(in_channels, hidden_dim, kernel_size, stride=2)
            )
        ...
        decoder_layers.append(
            ResidualBlock1D(
                hidden_dims_reversed[0], hidden_dims_reversed[0], kernel_size, stride=1
            )
        )
        ...
        for i, hidden_dim in enumerate(hidden_dims_reversed):
            ...
            out_channels = (
                hidden_dims_reversed[i] if i < len(hidden_dims_reversed) else 1
            )
            stride = 2 if i < len(hidden_dims_reversed) - 1 else 1
            decoder_layers.append(
                ResidualBlock1D(hidden_dim, out_channels, kernel_size, stride=stride)
            )
...
def conv1d_vae_loss_function(
    reconstruction, x, mu, logvar, beta=1.0, layer_weights=None, tv_weight=0.01
):
    """
