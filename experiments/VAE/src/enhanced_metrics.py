...
def compute_weighted_metrics(real_profiles: np.ndarray, generated_profiles: np.ndarray, 
                           depths: np.ndarray, weights: np.ndarray) -> Dict[str, float]:
...
def compute_vs30_metrics(real_vs30: List[float], generated_vs30: List[float]) -> Dict[str, float]:
    ...
    if len(real_vs30_clean) == 0 or len(gen_vs30_clean) == 0:
        return {'ks_statistic': 1.0, 'ks_pvalue': 0.0, 'mean_ratio': 1.0, 'std_ratio': 1.0}
...
def compute_depth_wise_metrics(real_profiles: np.ndarray, generated_profiles: np.ndarray,
...
def plot_comprehensive_evaluation(real_profiles: np.ndarray, generated_profiles: np.ndarray,
                               depths: np.ndarray, vs30_metrics: Dict[str, float],
                               save_path: str = None):
    ...
    for i in range(min(5, real_profiles.shape[0])):
        ...
        axes[0, 1].plot(generated_profiles[i], depths[:-1], 'r--', alpha=0.7, linewidth=1)
    ...
    axes[0, 2].fill_betweenx(layer_mid_depths, real_mean - real_std, real_mean + real_std, 
                             alpha=0.3, color='blue', label='Real ±1σ')
    axes[0, 2].fill_betweenx(layer_mid_depths, gen_mean - gen_std, gen_mean + gen_std, 
...
# Import the calculate_vs30 function from utils
