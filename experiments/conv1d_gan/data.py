...
class VsProfilesDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        padded = np.pad(
            seq, (0, pad_len), mode="constant", constant_values=self.pad_value
        )
...
if __name__ == "__main__":
    ...
    for i, (x, m) in enumerate(loader):
        print(
            f"Batch {i}: x.shape={x.shape}, mask.shape={m.shape}, first seq lengths={[int(mask.sum().item()) for mask in m]}"
        )
        if i >= 1:
            break
