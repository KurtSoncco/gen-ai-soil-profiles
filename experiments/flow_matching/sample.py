...
def load_latest_checkpoint(dir_path: str) -> str:
    ...
    # Sort by step number
    files.sort(key=lambda x: int(x.split("_")[-1].split(".")[0]) if x != "checkpoint_final.pt" else float('inf'))
    return os.path.join(dir_path, files[-1])


def main() -> None:
    parser = argparse.ArgumentParser(description="Generate Vs profiles using trained FFM model")
    parser.add_argument("--num_samples", type=int, default=16, help="Number of samples to generate")
    parser.add_argument("--checkpoint_path", type=str, default=None, help="Path to checkpoint file")
    parser.add_argument("--ode_steps", type=int, default=None, help="Number of ODE integration steps")
    parser.add_argument("--output_path", type=str, default=None, help="Output path for samples")
    ...
    _, max_length, dataset = create_dataloader(batch_size=cfg.batch_size, num_workers=0, shuffle=False)
    ...
    print(f"Loaded {cfg.model_type.upper()} model with {sum(p.numel() for p in model.parameters()):,} parameters")
