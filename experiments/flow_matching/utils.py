...
def compute_vs30_distribution(profiles: np.ndarray, depth_step: float = 0.5) -> np.ndarray:
...
def log_vs30_metrics(model, cfg, device, step, output_dir, real_vs30, avg_samples_per_meter, max_length, wandb=None):
    ...
    print(f"[debug] Generated samples range: [{np.min(generated_samples_np):.3f}, {np.max(generated_samples_np):.3f}]")
    ...
    if sample_range < 10:  # Very small range indicates poor training
        print(f"[warning] Generated samples have very small range ({sample_range:.3f}). Model may not be properly trained.")
    
    gen_vs30 = compute_generated_vs30(generated_samples_np, avg_samples_per_meter)
    
    # Debug: Print Vs30 statistics
    print(f"[debug] Generated Vs30 range: [{np.min(gen_vs30):.3f}, {np.max(gen_vs30):.3f}]")
    print(f"[debug] Generated Vs30 mean: {np.mean(gen_vs30):.3f}")
    print(f"[debug] Real Vs30 range: [{np.min(real_vs30):.3f}, {np.max(real_vs30):.3f}]")
    ...
    if wandb is not None:
        try:
            ...
            wandb.log({
                ...
                "histograms/generated_samples": wandb.Histogram(generated_samples_np.flatten()),
            })
    ...
    if step % 50 == 0 or step == cfg.num_steps - 1:
        ...
        plt.hist(real_vs30, bins=bins, alpha=0.6, label="real", density=True, color="blue")
        plt.hist(gen_vs30, bins=bins, alpha=0.6, label="generated", density=True, color="red")
        plt.xlabel("Vs30 (m/s)")
        plt.ylabel("Density")
        ...
        plt.title(f"Generated Sample Distribution\nRange: [{np.min(generated_samples_np):.3f}, {np.max(generated_samples_np):.3f}]")
        plt.grid(True, alpha=0.3)
        
        # Add Vs30 comparison
        plt.subplot(2, 2, 3)
        plt.scatter(range(len(gen_vs30)), gen_vs30, alpha=0.6, s=10, color="red", label="Generated")
        plt.scatter(range(len(real_vs30[:len(gen_vs30)])), real_vs30[:len(gen_vs30)], alpha=0.6, s=10, color="blue", label="Real")
...
"""
        ...
        plt.text(0.1, 0.9, stats_text, transform=plt.gca().transAxes, fontsize=10, verticalalignment="top", fontfamily="monospace")
        
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, f"vs30_hist_{step}.png"), dpi=300, bbox_inches="tight")
        plt.close()
...
def plot_profile_comparison(real_profiles: np.ndarray, generated_profiles: np.ndarray, 
                          output_dir: str, step: int, max_profiles: int = 8) -> None:
    ...
    for i in range(min(max_profiles, real_profiles.shape[0])):
        ax1.plot(real_profiles[i, 0, :], depth_axis, alpha=0.7, color='blue', linewidth=1)
    ...
    for i in range(min(max_profiles, generated_profiles.shape[0])):
        ax2.plot(generated_profiles[i, 0, :], depth_axis, alpha=0.7, color='red', linewidth=1)
    ...
    ax4.bar(x - width/2, [stats_data[k][0] for k in stats_data.keys()], 
            width, label='Real', alpha=0.7, color='blue')
    ax4.bar(x + width/2, [stats_data[k][1] for k in stats_data.keys()], 
            width, label='Generated', alpha=0.7, color='red')
...
def plot_generation_trajectory(trajectory: np.ndarray, output_dir: str, step: int) -> None:
