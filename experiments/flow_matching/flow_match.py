...
class SoilProfileDataset(Dataset):
    ...
    def __init__(
        self, num_samples=Config.N_MOCK_PROFILES, points=Config.PROFILE_POINTS
    ):
        self.points = points
        ...
        self.profiles_normalized = (all_data - self.min_val) / (
...
if __name__ == "__main__":
    ...
    for i in range(4):
        plt.plot(
            generated_samples[i, 0, :], depth_axis, label=f"Generated Sample {i + 1}"
        )
    ...
    sample_trajectory = (
        sample_trajectory * (dataset.max_val - dataset.min_val)
    ) + dataset.min_val
    plt.imshow(
        sample_trajectory,
        aspect="auto",
