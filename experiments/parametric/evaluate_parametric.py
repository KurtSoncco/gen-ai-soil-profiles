...
class ParametricProfileEvaluator:
    ...
    def compare_profiles(self, real_profiles, generated_profiles, model_name):
        ...
        comparison['vs100_ks_statistic'] = stats.ks_2samp(real_vs100, gen_vs100).statistic
        ...
        comparison['vs500_ks_statistic'] = stats.ks_2samp(real_vs500, gen_vs500).statistic
    ...
    def plot_comprehensive_comparison(self, real_profiles, generated_profiles_dict, 
                                    model_names, save_path):
        """
        ...
        for i, model_name in enumerate(model_names):
            ...
            for j in range(min(20, real_profiles.shape[0])):
                ax.plot(real_profiles[j], self.layer_centers, 'b-', alpha=0.3, linewidth=0.5)
            
            # Plot generated profiles
            for j in range(min(20, generated_profiles.shape[0])):
                ax.plot(generated_profiles[j], self.layer_centers, 'r-', alpha=0.3, linewidth=0.5)
        ...
        for i, model_name in enumerate(model_names):
            ...
            ax.fill_betweenx(self.layer_centers, real_mean - real_std, real_mean + real_std, 
                           alpha=0.3, color='blue')
            
            ax.plot(gen_mean, self.layer_centers, 'r-', label='Generated Mean', linewidth=2)
            ax.fill_betweenx(self.layer_centers, gen_mean - gen_std, gen_mean + gen_std, 
                           alpha=0.3, color='red')
        ...
        for i, model_name in enumerate(model_names):
            ...
            ax.plot(real_grad_mean, self.layer_centers[:-1], 'b-', label='Real', linewidth=2)
            ax.plot(gen_grad_mean, self.layer_centers[:-1], 'r-', label='Generated', linewidth=2)
    ...
    def plot_parameter_analysis(self, fitted_parameters_dict, generated_parameters_dict, 
                              fitters, save_path):
        """
        ...
        for i, (model_name, fitted_params) in enumerate(fitted_parameters_dict.items()):
            ...
            for j, name in enumerate(param_names):
                ax_generated.hist(generated_params[:, j], bins=30, alpha=0.7, label=name)
...
def load_results(results_dir):
    ...
    for model_file in results_dir.glob("*.pkl"):
        if "generator" in model_file.name:
            ...
            # Load generator
            generator = ParameterGenerator(model_type='gmm' if gen_type == 'gmm' else 'mlp')
...
def main():
    ...
    logging.info("Note: This is a demonstration script. In practice, load real data here.")
    ...
    for model_name, generated_profiles in generated_profiles_dict.items():
        comparison = evaluator.compare_profiles(real_profiles, generated_profiles, model_name)
