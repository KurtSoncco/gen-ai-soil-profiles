...
from experiments.VAE.src.preprocessing import (  # noqa: E402
    preprocess_data,
    standardize_and_create_tts_profiles,
)
from experiments.VAE.src.utils import calculate_vs30, tts_to_Vs  # noqa: E402
...
def convert_to_vs_profiles(profiles_dict, num_layers=100, max_depth=2000):
    ...
    logging.info(
        f"Converted to {vs_profiles.shape[0]} Vs profiles with {vs_profiles.shape[1]} layers"
    )
...
def fit_parametric_models(
    vs_profiles, standard_depths, model_types=["exponential", "power_law", "layered"]
):
    ...
    for model_type in model_types:
        ...
        for i, name in enumerate(param_names):
            ...
            logging.info(
                f"  {name}: mean={np.mean(param_values):.2f}, "
...
def evaluate_parametric_models(
    fitters, fitted_parameters, vs_profiles, standard_depths
):
    ...
    for model_type, fitter in fitters.items():
        ...
        recon_vs30 = [
            calculate_vs30(p, standard_depths) for p in reconstructed_profiles
        ]
        ...
        logging.info(
            f"  {model_type} - Vs30 MSE: {vs30_mse:.4f}, Vs30 MAE: {vs30_mae:.4f}"
        )
...
def generate_new_profiles(generators, fitters, n_new_profiles=1000):
    ...
    for model_type, model_generators in generators.items():
        ...
        for gen_type, generator in model_generators.items():
            ...
            generated_vs_profiles = fitters[model_type].generate_profiles(
                generated_params
            )
...
def plot_results(
    ...
    for i, (model_type, results) in enumerate(evaluation_results.items()):
        ...
        for j in range(min(10, recon_profiles.shape[0])):
            ax.plot(
                recon_profiles[j], standard_depths[1:], "r-", alpha=0.3, linewidth=0.5
            )
    ...
    plt.tight_layout()
    plt.savefig(
        save_dir / "parametric_model_comparison.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
    ...
    for model_type, model_generators in generated_profiles.items():
        for gen_type, results in model_generators.items():
            ...
            ax.set_title(
                f"{model_type.title()} + {gen_type.upper()}\nGenerated Profiles"
            )
            ax.set_xlabel("Vs (m/s)")
            ax.set_ylabel("Depth (m)")
            ax.set_ylim(max(standard_depths), 0)
    ...
    for model_type, model_generators in generated_profiles.items():
        ...
        plt.tight_layout()
        plt.savefig(
            save_dir / f"{model_type}_parameter_distributions.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
...
def main():
    ...
    try:
        ...
        vs_profiles, standard_depths = convert_to_vs_profiles(
            profiles_dict, NUM_LAYERS, MAX_DEPTH
        )

        # 3. Fit parametric models
        fitted_parameters, fitters = fit_parametric_models(
            vs_profiles, standard_depths, MODEL_TYPES
        )

        # 4. Evaluate parametric models
        evaluation_results = evaluate_parametric_models(
            fitters, fitted_parameters, vs_profiles, standard_depths
        )
