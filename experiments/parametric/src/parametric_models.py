...
class ExponentialModel(ParametricProfileModel):
    ...
    def fit(self, vs_profile: np.ndarray) -> Dict[str, float]:
        ...
        def objective(params):
            ...
            predicted = self._exponential_function(self.layer_centers, vs_shallow, vs_deep, z_transition)
    ...
    def generate(self, params: Dict[str, float]) -> np.ndarray:
        ...
        return self._exponential_function(self.layer_centers, vs_shallow, vs_deep, z_transition)
    
    def _exponential_function(self, z: np.ndarray, vs_shallow: float, vs_deep: float, z_transition: float) -> np.ndarray:
...
class PowerLawModel(ParametricProfileModel):
    ...
    def fit(self, vs_profile: np.ndarray) -> Dict[str, float]:
        ...
        def objective(params):
            ...
            predicted = self._power_law_function(self.layer_centers, vs_shallow, z_ref, alpha)
            return np.sum((vs_profile - predicted) ** 2)
    ...
    def _power_law_function(self, z: np.ndarray, vs_shallow: float, z_ref: float, alpha: float) -> np.ndarray:
