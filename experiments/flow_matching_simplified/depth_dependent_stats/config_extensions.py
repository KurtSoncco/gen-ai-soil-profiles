...
class DepthStatsConfig:
    ...
    vertical_corr_weight: float = 0.025  # Loss weight for matching vertical correlations (increased from 0.01)
    ...
    min_bin_count: int = 50  # Minimum samples per bin to include in loss (avoid noisy targets)
    mean_weight_ratio: float = 2.0  # Weight ratio: mean_loss / std_loss (emphasize mean trend correction)
    short_lag_weight: float = 2.0  # Extra weight for short lags (1-2m) in correlation loss
    
    # Paths for pre-computed statistics
    target_stats_path: str = "real_depth_stats.pkl"  # Path to pre-computed depth statistics
...
def get_depth_stats_config(base_config: cfg_mod.Config = None) -> DepthStatsConfig:
    ...
    return DepthStatsConfig(
        ...
        correlation_lags=getattr(base_config, "correlation_lags", [1.0, 2.0, 5.0, 10.0]),
        max_depth_for_stats=getattr(base_config, "max_depth_for_stats", 500.0),
        target_stats_path=getattr(base_config, "target_stats_path", "real_depth_stats.pkl"),
        target_corr_path=getattr(base_config, "target_corr_path", "real_correlations.pkl"),
